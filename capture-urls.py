...
def get_timestamp_age(timestamp) -> Union[int, None]:
    try:
        return (
            datetime.utcnow() - datetime.strptime(timestamp, '%Y%m%d%H%M%S')
        ).days
    except ValueError:
        return None
...
def process_input_url(client, url, progress: Progress):
    ...
    log(f' - was last captured {last_capture_age} '
...
def capture_urls(progress: Progress):
    with httpx.Client(timeout=TIMEOUT) as client:
        ...
        while len(progress.pending_urls) > 0:
            for result in check_status(
                    client,
                    list(progress.pending_urls.values())
            ):
